    ...
    <>
      <section className="relative overflow-hidden border-b border-slate-200 pt-28 pb-[4rem] md:pt-32 md:pb-20 xl:pt-40 xl:pb-24">
        ...
        <div className="container-shell relative">
          <p className="text-sm text-slate-700/45">Home &gt; Blog &gt; {post.title}</p>
          <div className="mt-6 inline-flex rounded-full border border-accent/20 bg-accent/10 px-3 py-1 text-label text-accent">
            {post.category}
          </div>
          <h1 className="mt-5 max-w-5xl font-bricolage text-[clamp(2.6rem,10vw,5.4rem)] font-semibold leading-[0.94] tracking-[-0.05em] text-slate-900">
            {post.title}
          </h1>
          <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-slate-700/55">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="font-semibold text-slate-900">By {post.author}</p>
                <p>
      ...
      <section className="section-padding bg-bg">
        <div className="container-shell">
          <div className="overflow-hidden rounded-[34px] border border-slate-200">
            <Image
              src={post.coverImage}
              alt={post.title}
              width={1600}
          ...
          <div className="mt-12 grid gap-10 xl:grid-cols-[minmax(0,1fr)_18rem] xl:gap-12">
            <article className="prose colodam-prose max-w-none text-[17px] leading-8">
            ...
            <aside className="surface-panel h-fit rounded-[28px] p-6 xl:sticky xl:top-28">
              <p className="text-label text-slate-700/40">Reading this for a reason?</p>
              ...
              <p className="mt-4 text-sm leading-7 text-slate-700/58">
                Book a brand strategy call if this article points to an issue your leadership team is
      ...
      <section className="section-padding border-t border-slate-200 bg-[#f4f7ff]">
        <div className="container-shell">
          <h2 className="font-bricolage text-[clamp(2rem,4vw,2.25rem)] font-semibold text-slate-900">
            Related reading
          </h2>
            ...
            {related.map((item) => (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <h3 className="mt-4 font-bricolage text-2xl font-semibold leading-tight text-slate-900">
                    {item.title}
                  </h3>
                  <p className="mt-3 text-sm leading-7 text-slate-700/60">{item.excerpt}</p>
                </div>
              </Link>
            ))}
          </div>
