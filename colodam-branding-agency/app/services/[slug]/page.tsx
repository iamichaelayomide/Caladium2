...
export default async function ServiceDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-slate-200 pb-20 pt-32 md:pb-24 md:pt-40">
        <div aria-hidden className="hero-glow absolute inset-0 opacity-70" />
        <div className="container-shell relative">
          <p className="text-sm text-slate-700/45">Home &gt; Services &gt; {service.name}</p>
          <p className="mt-6 text-label text-accent">{service.name}</p>
          <h1 className="mt-4 max-w-5xl font-bricolage text-[clamp(3.2rem,6vw,5.8rem)] font-semibold leading-[0.92] tracking-[-0.05em] text-slate-900">
            {service.tagline}
          </h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-700/66">{service.summary}</p>
          <Link
            href="/contact"
      ...
      <section className="section-padding-tight bg-bg">
        <div className="container-shell">
          <div className="relative overflow-hidden rounded-[34px] border border-slate-200">
            <Image
              src={service.image}
              alt={service.name}
              width={1600}
      ...
      <section className="section-padding bg-bg">
        <div className="container-shell grid gap-10 xl:grid-cols-[1.08fr_0.92fr] xl:gap-12">
          <div className="space-y-6 text-base leading-8 text-slate-700/66 md:text-[1.05rem]">
            {service.detailParagraphs.map((paragraph) => (
              <p key={paragraph}>{paragraph}</p>
            ))}
          </div>
          <aside className="surface-panel h-fit rounded-[30px] p-6 xl:sticky xl:top-28">
            <p className="text-label text-slate-700/40">Deliverables</p>
            <ul className="mt-6 space-y-3">
              {service.features.map((feature) => (
                <li key={feature} className="flex gap-3 text-sm leading-7 text-slate-700/60">
      ...
      <section className="section-padding border-y border-slate-200 bg-[#f4f7ff]">
        <div className="container-shell">
          <h2 className="font-bricolage text-[clamp(2.3rem,4vw,3.6rem)] font-semibold leading-[0.98] tracking-[-0.04em] text-slate-900">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {service.subServices.map((item, index) => (
              <article key={item.title} className="surface-panel overflow-hidden rounded-[30px]">
                ...
                />
                <div className="p-6">
                  <h3 className="font-bricolage text-[1.85rem] font-semibold text-slate-900">
                    {item.title}
                  </h3>
                  <p className="mt-4 text-base leading-8 text-slate-700/60">{item.description}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="section-padding bg-bg">
        <div className="container-shell">
          <h2 className="font-bricolage text-[clamp(2.2rem,4vw,3.4rem)] font-semibold leading-[0.98] tracking-[-0.04em] text-slate-900">
            Related services
          </h2>
          <div className="mt-10 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {related.map((item) => (
              ...
              >
                ...
                <h3 className="mt-4 font-bricolage text-[1.85rem] font-semibold leading-[1.03] text-slate-900">
                  {item.tagline}
                </h3>
                <p className="mt-4 text-base leading-8 text-slate-700/60">{item.summary}</p>
              </Link>
            ))}
          </div>
          <div className="surface-panel mt-12 rounded-[32px] p-6 text-slate-900 md:p-8 xl:p-10">
            <h2 className="font-bricolage text-[clamp(2rem,4vw,2.25rem)] font-semibold">
              Ready to move on this service?
            </h2>
            <p className="mt-4 max-w-2xl text-base leading-8 text-slate-700/64">
